    ...
    def flatten(self, data, prefix="", result=None):
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                ...
                # FIX: Leerzeichen in Keys → Underscore (z.B. "Power DC" → "power_dc")
                normalized_key = key.replace(" ", "_")
                new_prefix = f"{prefix}_{normalized_key}" if prefix else normalized_key
                flat_key = new_prefix.lower()

                # Einfache Zahlenwerte direkt speichern (z.B. limit_relative, limit_absolute)
                if isinstance(value, (int, float)):
                    if flat_key in result:
                        _LOGGER.warning("Schlüsselkonflikt beim Flatten: %s wird überschrieben", flat_key)
                    result[flat_key] = {"value": value, "unit": None}
                    continue
